...
// POST investigate
app.post('/investigate/:meterId', async (req, res) => {
  try {
    console.log(`Sending investigation request to ML service for meter ${req.params.meterId}`);
    const mlResponse = await axios.post(`${ML_SERVICE_URL}/investigate/${req.params.meterId}`, req.body, {
      headers: {
    ...
    console.error(`Error during investigation for meter ${req.params.meterId}:`, error.message);
    
    if (error.response && error.response.data) {
       return res.status(error.response.status || 500).json(error.response.data);
    }
    res.status(500).json({ error: 'Failed to communicate with ML service for investigation' });
  }
});
